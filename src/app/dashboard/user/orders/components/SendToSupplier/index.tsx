...
export default function SendToSupplier({ closePopup }: { closePopup?: any }) {
  ...
  const filteredSuppliers =
    Suppliers?.data?.filter(
      (el: { supplier_type: string | null }) =>
        el?.supplier_type === selectedSupplierType
    ) || [];
  ...
  const cleaned = products.filter(
    (item: { productName: any; quantity: any }) =>
